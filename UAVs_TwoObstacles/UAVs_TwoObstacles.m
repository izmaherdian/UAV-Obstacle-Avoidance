%% UAVs_TwoObstacles - Simulasi Penghindaran Dua Rintangan Bergerak UAV Menggunakan CBF
% Script ini mensimulasikan gerak UAV 3D mengikuti trajektori heliks sembari
% menghindari dua rintangan bergerak menggunakan Control Barrier Functions (CBF).
...
%% Global Parameters
global A B Kp Kd mu r f v R Ox Oy Oz
...
% Kecepatan translasi sumbu z
v = 2;

% Radius trajektori heliks
R = 15;
...
% 1. Plot Trajektori 3D Robot & Rintangan
figure(1);
...
function [dx, Xd, Yd, Zd, Ux, Uy, Uz, Ob1, Ob2, h] = simulation(t, x)
    ...
    % Trajektori Referensi Heliks (Feedforward)
    yref = [Ox + R*cos(f*t);
            Oy + R*sin(f*t);
            Oz + v*t];

    yref_dot = [-R*f*sin(f*t);
                 R*f*cos(f*t);
                 v];

    yref_dd = [-R*(f^2)*cos(f*t);
               -R*(f^2)*sin(f*t);
                0];
